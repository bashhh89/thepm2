  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-50 overflow-hidden">
      ...
      <div className="mt-auto pt-4 border-t border-zinc-800">
        <button 
          onClick={() => signOut()}
          className="flex items-center gap-3 w-full p-3 text-zinc-400 hover:text-white hover:bg-zinc-800 rounded-md transition-colors"
        >
          <FiLogOut className="h-5 w-5" />
          <span className={cn("transition-opacity", isExpanded ? "opacity-100" : "opacity-0")}>Sign Out</span>
        </button>
      </div>
    </aside>
  )
