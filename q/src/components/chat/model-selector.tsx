...
export function ModelSelector({ 
  ...
  const handleSelectChange = (value: string) => {
    console.log('ModelSelector - Selected value:', value);
    
    // Find the selected model by ID directly from the TEXT models list
    const selectedModel = MODEL_LIST.TEXT.find(m => m.id === value);
    
    // Log the selected model for debugging
    console.log('ModelSelector - Selected model object:', selectedModel);
    
    // Invoke callback with the complete model object
    if (selectedModel && onModelSelect) {
      console.log('ModelSelector - Calling onModelSelect with:', selectedModel);
      onModelSelect(selectedModel);
    } else {
      console.error('ModelSelector - Failed to find model with ID:', value);
    }
  };
  ...
  return (
    <Select value={selectedModel?.id} onValueChange={handleSelectChange}>
      ...
      <SelectContent 
        className="bg-zinc-800 border-zinc-700 text-zinc-100 max-h-[400px]"
        onCloseAutoFocus={(e) => {
          // Prevent the focus behavior from interfering with selection
          e.preventDefault();
        }}
      >
        <div className="p-2">
